...
/**
 * FallingPattern — animated falling rain/line background effect.
 *
 * Props:
 * @param {string}  color            — Primary color of falling elements (default: var(--accent))
 * @param {string}  backgroundColor  — Background fill color (default: var(--ink))
 * @param {number}  duration         — Animation duration in seconds (default: 150)
 * @param {string}  blurIntensity    — Backdrop blur for dot overlay (default: '1em')
 * @param {number}  density          — Dot overlay density / spacing multiplier (default: 1)
 * @param {string}  className        — Extra class names passed to the wrapper div
 */
export function FallingPattern({
  ...
  return (
    <div
      className={cn('falling-pattern-root', className)}
      style={{ position: 'relative', height: '100%', width: '100%', padding: '4px' }}
    >
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
